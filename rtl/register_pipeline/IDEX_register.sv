...
//branch_predictor_unit_signal
    input  logic [7:0]  i_pht_idxD,
    input  logic        i_pred_takenD,
    input  logic [31:0] i_pred_targetD,

    output logic [7:0]  o_pht_idxE,
    output logic        o_pred_takenE,
    output logic [31:0] o_pred_targetE
);
    ...
    always_ff @(posedge i_clk or negedge i_rst_n) begin
        if (!i_rst_n || i_flushE) begin
            ...
            alu_srcBE      <= alu_srcBD;
            o_instE        <= i_instD;
            o_pht_idxE     <= i_pht_idxD;
            o_pred_takenE  <= i_pred_takenD;
            o_pred_targetE <= i_pred_targetD; 
        end
    end
